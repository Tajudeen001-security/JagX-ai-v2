...
MAX_OUTPUT_TOKENS = int(os.environ.get("JAGX_MAX_OUTPUT_TOKENS", "1200"))
TEMPERATURE = float(os.environ.get("JAGX_TEMPERATURE", "0.35"))  # lower = more accurate
...
def generate_response(user_message: str, history: Optional[List] = None) -> str:
    ...
    if not result:
        return "I could not generate a response right now. Please try again in a few seconds."
